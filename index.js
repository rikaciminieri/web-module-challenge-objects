...
*/
console.log(createMenuItem('Cheeseburger', 10, 'Lunch'));
...
export const burger = {
  ...
  discount: function(personType) {
    if (personType === "teacher" || personType === "student") {return this.price*0.75}
    if (personType === "public") {return this.price*0.9}
  }
}
...
*/
reviews.push({name: "Rika", rating: 5, feedback: "Cute IG spot! Took cute pics with all the backdrops they have."})
console.log(reviews);
...
function getLastReview(arrOfObjs) {
  const lastReview = arrOfObjs[arrOfObjs.length - 1];
...
function getLongReviews(arr) {
    let longReviewArr = [];
    for (let i in arr) {
      let feedbackLength = arr[i].feedback.split(" ").length;
      if (feedbackLength > 15) {
        longReviewArr.push(arr[i]);
      }
    }
    return longReviewArr;
  }
